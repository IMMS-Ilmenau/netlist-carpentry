...
@pytest.fixture()
def simple_graph_with_or_chain() -> nx.DiGraph:
    g = nx.DiGraph()
    g.add_node("module.or1", ntype="INSTANCE", ntype_info="§or", raw_path="module.or1")
    g.add_node("module.or2", ntype="INSTANCE", ntype_info="§or", raw_path="module.or2")
    g.add_node("module.or3", ntype="INSTANCE", ntype_info="§or", raw_path="module.or3")
    g.add_node("module.and1", ntype="INSTANCE", ntype_info="§and", raw_path="module.and1")
    g.add_node("in1", ntype="PORT", ntype_info="input")
    g.add_node("out", ntype="PORT", ntype_info="output")
...
def test_is_gate_subtype_mismatch_and_nsubtype(or_config):
    g = nx.DiGraph()
    g.add_node("m.or1", ntype="INSTANCE", ntype_info="§and")
...
def test_is_gate_not_instance(or_config):
    g = nx.DiGraph()
    g.add_node("m.port", ntype="PORT", ntype_info="§or")
    assert co.is_gate(g, "m.port", or_config) is False


def test_is_gate_exclude_chains_flag(or_config):
    g = nx.DiGraph()
    g.add_node("m.or_chain1_L0_N0", ntype="INSTANCE", ntype_info="§or")
...
def test_build_chain_stops_at_multiple_successors(or_config):
    g = nx.DiGraph()
    g.add_node("m.or1", ntype="INSTANCE", ntype_info="§or")
    g.add_node("m.or2", ntype="INSTANCE", ntype_info="§or")
    g.add_node("m.or3", ntype="INSTANCE", ntype_info="§or")
...
def test_is_target_gate_uses_nsubtype_fallback(or_config):
    class DummyInst:
        instance_type = None
        nsubtype = "§or"
        raw_path = "m.or1"
    assert co.is_target_gate(DummyInst(), or_config) is True
...
def test_collect_output_wires_multiple_segments_raises(boundary_extractor):
    ...
    class Inst:
        raw_path = "m.i"
        ports: ClassVar[dict[str, list[tuple[int, Seg]]]] = {
            "Y": [(0, Seg("m.w1.0")), (1, Seg("m.w2.0"))]
        }
